...
public class Hobgoblin extends Monster {
	...
	public void planMonster() {
		...
		for (String b : Befehle) {
            if (b.equals(";NEU")) {
                if (this.getName().startsWith("Einheit")) this.Befehle.add("BENENNE EINHEIT " + HobGoblinName());
                return;
            }
		...
		//TODO Hungernde Hobgoblins greifen an (?)
                ...
                if (polizist != null) {
                    if (polizist.getCoords().equals(this.getCoords())) {
                        ...
                        if (gibAuswanderBefehle(false)) { // Reise nicht erzwingen
                            ...
                            new SysMsg("Hobgoblinpanik bei " + this + " in " + region + " (sind erwischt worden und können nicht fliehen)!");
        ...
        // sind andere Hobgoblins hier, mit einer niedrigeren ID als wir?
		for (Unit other : region.getUnits(this.getOwner())) {
			if ((other.getClass() == Hobgoblin.class) && (other.getNummer() < this.getNummer())) {
				if (Random.W(100) <= 50) {
                    // dann: auswandern
                    if (gibAuswanderBefehle(false)) { // Reise nicht erzwingen
		...
		if (players.isEmpty()) {
            ...
            if (this.canWalk()) {
                ...
                if (getItem(Silber.class).getAnzahl() < getPersonen() * 12 * 10) {
                    ...
                    new SysMsg("Hobgoblinpanik bei " + this + " in " + region + " (weniger als ein Jahresvorrat vorhanden)!");
			...
			int modus = getIntegerProperty("hobgoblin.modus", 0);
			if (modus == 0)	{
			...
			setProperty("hobgoblin.modus", 1 - modus);
    ...
    protected void gibSpaltungsBefehle(int personenGeben, int silberGeben, int rekrutieren) {
        ...
        Befehle.add("BENENNE EINHEIT " + HobGoblinName());
    ...
    /** 
	 * <p>Kümmert sich um das globale Hobgoblinwesen.</p>
	 *
	 * Aussetzen der Hobgoblins
	 * 
	 * dazu werden alle Inseln kleiner als settings -> "inselkennung.spieler" abgeklappert
	 * 1. ist auf einer Insel ein Hobgoblin -> weiter mit 5.
	 ...
	 * 5. Hobgoblins wandern willkürlich und suchen Spieler
	 * 6. abwechselnd (pro Runde halt)
	 *     -> Spieler wird beklaut
	 *     -> Hobgoblins lernen Hiebwaffen
	...
	public static void NeueRunde() {
		new ZATMsg("Monsterplanung: Hobgoblins");
		for(int insel = 0; insel < GameRules.getInselkennungSpieler() - 3; insel++) {
			if (Random.W(100) <= 20)spawnHobgoblinEVA(insel);
		}
	}
	
	private static void spawnHobgoblinEVA(int insel) {
		List<Region> regionen = Region.getInselRegionen(insel, true);
		
		// überprüfen ob Hobgoblins schon da sind
		boolean k = false;	// Hobgoblin
		boolean w = false;	// Wahrnehmung >= 10 auf der Insel vorhanden 
		for(Region r: regionen)	{
			if (k) break;
			for(Unit unit : r.getUnits()) {
				if (k) break;
				if (unit.getClass().equals(Hobgoblin.class)) k = true;
				if (unit.Talentwert(Wahrnehmung.class) >= 10) w = true;
			}
		}
		if (k) return;
		if (!w) {
			new Debug("Kein Wahrnehmer >T9 auf Insel #" + insel + " --> keine Hobgoblins.");
			return;
		}
	...
	private static Unit createKoboldEVA(int insel, Coords coords) {
		...
		Unit unit = null;
		if (coords == null)	{
			unit = Unit.CreateUnit("Hobgoblin", Codierung.fromBase36("dark"), regionen.get(Random.rnd(0, regionen.size())).getCoords());
		} else {
			unit = Unit.CreateUnit("Hobgoblin", Codierung.fromBase36("dark"), coords);
		}
	...
	protected static String HobGoblinName() {
	...
	/**
	 * setzt die entsprechenden Hobgoblin-Waffen
	...
	{
		return allowLernen(skill);	// Hobgoblin dürfen lernen
	}
        ...
        for (Message msg : messages) {
            ...
            if (txt.contains("kann") && txt.contains("silber") && txt.contains("stehlen")) {
                ...
                if (ids.size() == 2) {
                    ...
                    new SysErr("Hobgoblin.meldungenAuswerten() macht offenbar falsche Annahmen zum Meldungsformat (erfolgreicher Diebstahl)");
                }
                ...
                if (ids.size() == 1) {
                    ...
                    new SysErr("Hobgoblin.meldungenAuswerten() macht offenbar falsche Annahmen zum Meldungsformat (erfolgloser Diebstahl)");
                }
                ...
                if (ids.size() == 2) {
                    ...
                    new SysErr("Hobgoblin.meldungenAuswerten() macht offenbar falsche Annahmen zum Meldungsformat (beim Diebstahl ertappt)");
                }
            }
        }
    }
