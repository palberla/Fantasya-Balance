...
import de.x8bit.Fantasya.Atlantis.Ships.Trireme;
...
public abstract class Unit extends Atlantis implements Comparable {
	 ...
	 */
	public double rekrutierungsFaktor(Coords coords) { return 1.0; }
	...
	public void setTarnRasse(String value)
	{
		if (darfRasseTarnen())
		{
			...
			{
				try {
					Unit u = (Unit) Class.forName("de.x8bit.Fantasya.Atlantis.Units." + value.trim()).newInstance(); // Existiert die Rasse ueberhaupt?
					this.TarnRasse = value; // Ja, existiert
				} catch (InstantiationException | IllegalAccessException | ClassNotFoundException e) {
					new Fehler("Tarnung der Rasse " + value.trim() + " ist fehlgeschlagen. Rasse existiert nicht", this, this.getCoords());
					// Rasse existiert anscheinend nicht => Nicht tarnen als Standardwert.
				}
			}
		}
		else { new Fehler("Tarnung der Rasse wird von dieser Rasse " + this.getRasse() + " nicht beherrscht.", this, this.getCoords()); }
	}
	public boolean darfRasseTarnen() { return false; };
	...
	public boolean canFly()
	{
		...
		int flyelements = getItem(Pegasus.class).getAnzahl() + getItem(Greif.class).getAnzahl();
	...
	public boolean canWalkAnimals()
	{
		...
		// Kamele die das Talent Reiten benötigen
		int kamele = getItem(Kamel.class).getAnzahl() - getPersonen();
		if (kamele < 0) kamele = 0;
		// Alpakas die das Talent Reiten benötigen
		int alpakas = getItem(Alpaka.class).getAnzahl() - getPersonen();
	...
	public boolean Rekrutieren(int anzahl) {
		...
		hinzufuegenStartFertigkeiten(anzahl);
		getItem(Silber.class).setAnzahl(getItem(Silber.class).getAnzahl() - anzahl * getRekrutierungsKosten());
		r.setBauern(r.getBauern() - ( anzahl * (int) ( 1.0 / this.rekrutierungsFaktor(getCoords()))));
	...
	public boolean checkSegeln(ReiseVerb reiseVerb)
	{
		...
		if (this.Talentwert(Seefahrt.class) < ship.getKapitaenTalent()) {
		...
		for(Unit unit : region.getUnits()) {
			if ( /* unit.getOwner() == owner.getOwner() && */ unit.getSchiff() == ship.getNummer()) {
				talent += unit.Talentwert(Seefahrt.class) * unit.getPersonen();
			}
		}
		...
		if (ship.getClass() == Trireme.class) reiseVerb.setVerb("kommandiert die Rudersklaven");
