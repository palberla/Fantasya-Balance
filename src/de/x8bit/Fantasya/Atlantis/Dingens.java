...
public class Dingens extends Atlantis {
	...
	/** alle Rassen die Resourcen sparen */
	private ConstructionRace [] neededConstructionRace = null;
	public void setConstructionRaces(ConstructionRace [] neededConstructionRace) { this.neededConstructionRace = neededConstructionRace; }
	public ConstructionRace [] getConstructionRaces() { return neededConstructionRace; }
	...
	/**
	 * Berechnet die Vorteile durch die Gebäude und die Rasse
	 * @param item - dieses Item wird verwendet
	 * @param unit - diese Einheit produziert
	...
	{
		if (getConstructionCheats() == null && getConstructionRaces() == null) return 0; // keine Verbesserungen
		
		double faktor = 0.0; // Sparfaktor
		
		if (getConstructionCheats() != null) // Gebäude
		{
			if (unit.getGebaeude() > 0) // keine Gebäude -> kein Effekt
			{
				...
				if (building != null)
				{
					...
					{
						double matchValue = 0; // >0 wenn das Item weniger verbraucht wird
						if (building.getClass().equals(cc.getConstructionBuilding()))	// das Gebäude passt schon mal
						{
							...
							{
								if (item.equals(ci.getClazz())) matchValue = 1.0 / ci.getValue();
							}
							if (matchValue > 0) { faktor = matchValue * zukleinFaktor; };
		...
		if (getConstructionRaces() != null) // Rasse
		{
			for(ConstructionRace cr : getConstructionRaces())
			{
				double matchValue = 0;
				if (unit.getClass().equals(cr.getConstructionRace()))	// die Rasse passt schon mal
				{
					// Item testen
					for(ConstructionContainer ci : cr.getConstructionItems())
					{
						if (item.equals(ci.getClazz())) matchValue = (faktor > 0.0) ? (1.0 + (1.0 / ci.getValue())) : (1.0 / (ci.getValue() + 1));
					}
					if (matchValue > 0) { faktor = (faktor > 0.0) ? faktor * matchValue : matchValue; };
				}
			}
		}
