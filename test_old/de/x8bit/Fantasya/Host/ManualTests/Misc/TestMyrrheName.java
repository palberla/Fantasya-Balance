...
import de.x8bit.Fantasya.Atlantis.Items.Myrrhe;
...
//        List<Message> messages = null;
        ...
        for (Unit u:units) {
            ...
            if (tokens[1].equals("01") || tokens[1].equals("02") || tokens[1].equals("03")) {
				if (u.getItem(Myrrhe.class).getAnzahl() < 1) {
					retval = fail(tokens[1] + ": Hat keine Myrrhe.");
				}
				if (u.getItem(Myrrhe.class).getAnzahl() >= 50) {
					retval = fail(tokens[1] + ": Hat keine Myrrhe verkauft.");
				}
            }

            if (tokens[1].equals("04")) {
				if (u.getItem(Myrrhe.class).getAnzahl() > 0) {
